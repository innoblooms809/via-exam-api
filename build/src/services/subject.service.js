...
const createSubject = (body, createdBy) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [i, item] of input.entries()) {
        ...
        if (seen.has((0, academicNames_1.nameKey)(subjectName)))
            continue;
        seen.add((0, academicNames_1.nameKey)(subjectName));
    ...
    try {
        ...
        for (const cls of classes.sort((a, b) => (0, class_service_1.compareClassNames)(a.className, b.className))) {
            ...
            const byKey = new Map(current.map((s) => [(0, academicNames_1.nameKey)(s.subjectName), s]));
            for (const subject of subjects) {
                const found = byKey.get((0, academicNames_1.nameKey)(subject.subjectName));
                if (found && !found.isDeleted) {
                    skipped.push(classes.length > 1 ? `${found.subjectName} (${cls.className})` : found.subjectName);
                    continue;
                }
                if (found) {
                    yield found.update(Object.assign(Object.assign({ isDeleted: false, isActive: true, subjectName: subject.subjectName, totalMarks: subject.totalMarks, passingMarks: subject.passingMarks }, (subjectCode ? { subjectCode } : {})), (teacherId ? { teacherId } : {})), { transaction: t });
                }
                else {
...
const updateSubject = (subjectId, body, createdBy) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (subjectName !== subject.subjectName) {
            ...
            const clash = siblings.find((s) => (0, academicNames_1.nameKey)(s.subjectName) === (0, academicNames_1.nameKey)(subjectName));
            if (clash) {
        ...
        const marks = checkMarks((_a = body.totalMarks) !== null && _a !== void 0 ? _a : subject.totalMarks, (_b = body.passingMarks) !== null && _b !== void 0 ? _b : subject.passingMarks);
        if (!("totalMarks" in marks))
            return fail(http_status_1.default.BAD_REQUEST, marks.message);
