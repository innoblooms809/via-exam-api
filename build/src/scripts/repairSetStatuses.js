...
const manualResets = new Set(args
    .flatMap((arg, i) => (arg === "--reset" && args[i + 1] ? [args[i + 1]] : []))
    .map((v) => {
    const [examId, set] = v.split(":");
    return `${examId}:${String(set !== null && set !== void 0 ? set : "").trim().toUpperCase()}`;
}));
const RESET_FIELDS = {
    status: "DRAFT",
    submittedAt: null,
...
function main() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const papers = yield QuestionPaper_modal_1.default.findAll({ order: [["examId", "ASC"], ["paperSet", "ASC"]] });
        const answers = yield stander_answer_model_1.default.findAll();
        const touchedExams = new Set();
        let resetCount = 0;
        const answerFor = (p) => {
            var _a;
            return (_a = answers.find((a) => a.paperId === p.paperId)) !== null && _a !== void 0 ? _a : answers.find((a) => a.examId === p.examId && a.paperSet === p.paperSet);
        };
        for (const paper of papers) {
            ...
            if (locked && (missingAnswer || manual)) {
                ...
                if (APPLY) {
                    yield sequelize_1.sequelize.transaction((transaction) => __awaiter(this, void 0, void 0, function* () {
                        yield paper.update(RESET_FIELDS, { transaction });
                        if (answer)
                            yield answer.update(RESET_FIELDS, { transaction });
                    }));
                }
            }
            ...
            console.log(`${paper.examId}  Set ${paper.paperSet}  paper=${paper.status.padEnd(16)} answer=${((_a = answer === null || answer === void 0 ? void 0 : answer.status) !== null && _a !== void 0 ? _a : "—").padEnd(16)} ${action}`);
        }
        // Answer sheets whose set has no question paper.
        const orphans = answers.filter((a) => a.status !== "DRAFT" &&
        ...
        for (const answer of orphans) {
            ...
            if (APPLY)
                yield answer.update(RESET_FIELDS);
        }
        ...
        const examIds = args.includes("--recompute-all")
            ? new Set([...papers.map((p) => p.examId), ...answers.map((a) => a.examId)])
            : touchedExams;
        const resetKeys = new Set(papers
            .filter((p) => {
        ...
        for (const examId of examIds) {
            ...
            const next = exam.status === "Live" || exam.status === "Completed"
                ? exam.status
                : (0, exam_service_1.deriveExamStatus)(setStatuses, true);
            if (next !== exam.status) {
                examChanges++;
        ...
        if (unmatched.length)
            console.log(`--reset entries with no matching set: ${unmatched.join(", ")}`);
        yield sequelize_1.sequelize.close();
    });
}
main().catch((err) => __awaiter(void 0, void 0, void 0, function* () {
    console.error("Repair failed:", err);
    yield sequelize_1.sequelize.close().catch(() => undefined);
    process.exit(1);
}));
